...
public final class HexUtilities {
    ...
    /**
     * Converts the given string in HEX representation to a byte array, ignores whitespaces and
     * semicolons while doing so.
    ...
    public static byte[] decodeFromHex(String hexString) throws IllegalArgumentException{
        ...
        for (int i = 0; i < length; i += 2) {
            ...
            if (!((one >= 'a' && one <= 'f') || (one >= '0' && one <= '9')) || !((two >= 'a' && two <= 'f') || (two >= '0' && two <= '9'))) {
